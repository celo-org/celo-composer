...
export async function getChatsByWalletAddress({
  ...
  try {
    ...
    let filteredChats: Array<Chat> = [] as any;
    ...
    throw new ChatSDKError(
      "bad_request:database",
      "Failed to get chats by wallet address",
    );
  }
}
export async function getOrCreateUserByWalletAddress({
  ...
  try {
    ...
    if (existing) {
      // Normalize stored walletAddress to lowercase if needed
    ...
    throw new ChatSDKError(
      "bad_request:database",
      "Failed to get or create user by wallet address"
    );
  }
}
...
// Votes removed: voteMessage and getVotesByChatId were deprecated along with Vote tables.
...
export async function reassignChatToUser({
  chatId,
  userId,
}: {
  chatId: string;
  ...
  try {
    ...
    throw new ChatSDKError(
      "bad_request:database",
      "Failed to reassign chat to user"
    );
  }
}
